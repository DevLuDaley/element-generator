...
const ConverterPage = () => {
    ...
    return (
        ...
        >
    { optionsArray ?
        optionsArray.map(option => <MenuItem value={option.value} key={Math.floor(Math.random() * 10) +50}>
            {option.k}
        </MenuItem>)
        : "null"
    }
        {console.log('🚀 ~ file: ConverterPage.js ~ line 45 ~ ConverterPage ~ option.id', optionsArray[0].index)}
        </Select>
      </FormControl>
